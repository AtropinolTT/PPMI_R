...
LEDD_Concomitant_Medication_Log <- mutate(LEDD_Concomitant_Medication_Log, STARTDT = as.Date(paste("01/", as.character(STARTDT)), "%d/%m/%Y"))

# Add 1 month to the STOPDT because we assume that the dosage is taken until the end of the month, hence the change takes effect from the start of the next month
LEDD_Concomitant_Medication_Log <- mutate(LEDD_Concomitant_Medication_Log, STOPDT = as.Date(paste("01/", (as.integer(str_sub(STOPDT, 1, 2))%%12+1), "/", as.integer(str_sub(STOPDT, 4, 7))+(as.integer(str_sub(STOPDT, 1, 2))+1)%/%13, sep=""), "%d/%m/%Y"))
...
LEDD <- unique(rbind(LEDD_Concomitant_Medication_Log %>% select(PATNO, STARTDT),
                     LEDD_Concomitant_Medication_Log %>% select(PATNO, STARTDT = STOPDT) %>% filter(!is.na(STARTDT)))) %>% arrange (PATNO, STARTDT)

# Get the base L-Dopa value at each point in time
LDOPA_VALUES <- LEDD_Concomitant_Medication_Log[str_detect(LEDD_Concomitant_Medication_Log$LEDTRT,regex("LEV|DOP|RYTA|SINEMET|STANEK|STALEVO|INBRIJA|ISICOM|NACOM|PROLOPA",ignore_case=TRUE)),] %>% select(PATNO,STARTDT,STOPDT,LEDD,LEDDSTRMG,LEDDOSE,LEDDOSFRQ) %>%

# Also calculate the LEDD value from base medication details, if available, in case not populated in LEDD column
mutate(LEDD3 = case_when (is.numeric(LEDDSTRMG) ~ (LEDDSTRMG * LEDDOSE * LEDDOSFRQ) , TRUE ~ 0.0)) %>%
...
# If the drug is levodopa and the LEDD value is not available, attempt to derive it from base medication details
LEDD_VALUES <- mutate(LEDD_Concomitant_Medication_Log, LEDD3 = case_when(str_detect(LEDD_Concomitant_Medication_Log$LEDTRT,regex("LEV|DOP|RYTA|SINEMET|STANEK|
STALEVO|INBRIJA|ISICOM|NACOM|PROLOPA",ignore_case=TRUE)) ~ (LEDDSTRMG * LEDDOSE * LEDDOSFRQ) , TRUE ~ 0.0)) %>%
  mutate(LEDD2 = case_when (!is.na(as.double(as.character(LEDD))) ~ as.double(as.character(LEDD)) , TRUE ~ LEDD3)) 
